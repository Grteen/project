...
int main(void) {
    ...
    table *tb = new table();
    tb->tb_struct.tb_type[0] = "INT";
    tb->tb_struct.tb_tpname[0] = "1name";
    tb->tb_struct.tb_tpcomment[0] = "the first name";
    tb->tb_struct.tb_tpattr[0] = "unique,not null";
    tb->tb_struct.tb_type[1] = "INT";
    tb->tb_struct.tb_tpname[1] = "2name";
    tb->tb_struct.tb_tpcomment[1] = "the second name";
    tb->tb_struct.tb_tpattr[1] = "";
    tb->tb_struct.tb_type[2] = "INT";
    tb->tb_struct.tb_tpname[2] = "3name";
    tb->tb_struct.tb_tpcomment[2] = "the third name";
    tb->tb_struct.tb_tpattr[2] = "unique";
    tb->record[0] = "1:2:3";
    tb->record[1] = "1:2:3";
    tb->record[2] = "1:2:3";

    create_tb("6" , tb->tb_struct);
    tb->record_num = 3;
    tb->tb_struct.type_num = 3;
    tb->tb_name = "6";
    write_tables(tb);
    tb = read_tables("6");
    change_field("6" , "2name" , "newname" , "YEAR" , "new test" , "");
    drop_field("6" , "newname");
    tb = read_tables("6");
    tb->tb_struct.process_attr();
    rename_table("6" , "new666");
    drop_table("new666" , IF_EXIST);
    int size = 0;
    std::string * res = show_tables(&size);
    for (int i = 0 ; i < size ; i++)
        std::cout << res[i] << std::endl;
    // for (int i = 0 ; i < 3 ; i++) {
    //     std::cout << tb->tb_struct.tb_type[i] << std::endl;
    //     std::cout << tb->tb_struct.tb_tpname[i] << std::endl;
    //     std::cout << tb->tb_struct.tb_tpcomment[i] << std::endl;
    //     std::cout << tb->record[i] << std::endl;
    //     std::cout << tb->recordoff[i] << std::endl;
    //     std::cout << tb->tb_struct.tb_attrnum[i] << std::endl;
    //     std::cout << tb->tb_struct.tb_tpattr[i] << std::endl;
    //     for (int j = 0 ; j < tb->tb_struct.tb_attrnum[i] ; j++)
    //         std::cout << tb->tb_struct.tb_proattr[i][j] << std::endl;
    // }
}
